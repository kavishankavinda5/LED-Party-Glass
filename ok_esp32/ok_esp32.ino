  ...
  switch (buttonPushCounter) {
    ...
    case 1:
      buttonPushCounter == 1;
      {
        Vu3();
        //soundp2c1();
        //vun1();  // sound react solid red pattern 1
        //vu();  // sound react pattern 1
        break;
      }
    ...
    case 2:
      buttonPushCounter == 2;
      {
        Vu4();
        //soundp2c2();
        //vun2();  // sound react solid red pattern 1
        //Vu3();  // sound react pattern 2
        break;
      }
    ...
    case 3:
      ...
      {
        soundp1c1();
        //Vu3();  // sound react solid red pattern 1
        //vu2();  // sound react pattern 1
        break;
      }
...
void pattern2() {
  ...
  sinelon();
  if (digitalRead(buttonPin) != lastButtonState)  // <------------- add this
    return;                                       // <------------ and this                           // Call our sequence.
  delay(5);
  show_at_max_brightness_for_power();

  // Power managed display of LED's.
}  // loop()
...
void vun1() {
  ...
  //--------------------------------------------------------------------------------------------------------------------
  // Color pixels based on set to one color
  for (i = 0; i < N_PIXELS; i++) {
    if (i >= height) strip.setPixelColor(i, 0, 0, 0);
    else strip.setPixelColor(i, strip.Color(255, 0, 0));  // set color to solid red
  }

  // Draw peak dot
  //if (peak > 0 && peak <= N_PIXELS - 1) strip.setPixelColor(peak, strip.Color(0, 255, 0));
  //if (peak > 0 && peak <= N_PIXELS - 1) strip.setPixelColor(peak, strip.Color(255, 0, 0));
  ...
  for (i = 0; i < N_PIXELS; i++) {
  if (i >= height) strip.setPixelColor(i, 0, 0, 0);
  else if (i < N_PIXELS / 2) strip.setPixelColor(i, strip.Color(0, 0, 255)); // set color to blue for first half of strip
  else strip.setPixelColor(i, strip.Color(0, 255, 0)); // set color to green for second half of strip
  }
...
//-------------------------------------------------------------------------------------------
  ...
  for (i = 0; i < N_PIXELS_HALF; i++) {
    if (i >= height) {
      ...
      //strip.setPixelColor(N_PIXELS_HALF - i - 1, color);
      //strip.setPixelColor(N_PIXELS_HALF + i, color);
      strip.setPixelColor(N_PIXELS_HALF - i - 1, 0, 50, 0);
      strip.setPixelColor(N_PIXELS_HALF + i, 0, 50, 0);
    }
  }*/
  //-------------------------------------------------------------------------------------------
  // Color pixels based on rainbow gradient
...
// Draw peak dot
  if (peak > 0 && peak <= N_PIXELS_HALF - 1) {
    ...
    //strip.setPixelColor(N_PIXELS_HALF - peak - 1, color);
    //strip.setPixelColor(N_PIXELS_HALF + peak, color);
    strip.setPixelColor(N_PIXELS_HALF - peak - 1, 50, 0, 0);
    strip.setPixelColor(N_PIXELS_HALF + peak, 0, 0, 50);
  }
...
void vun3() {
  ...
  */
  for (i = 0; i < N_PIXELS; i++) {
    if (i >= height) strip.setPixelColor(i, 0, 0, 0);
    else if (i < N_PIXELS / 4) strip.setPixelColor(i, strip.Color(0, 245, 255));       // set color to blue for first quarter of strip
    else if (i < N_PIXELS / 2) strip.setPixelColor(i, strip.Color(252, 231, 0));       // set color to red for second quarter of strip
    else if (i < N_PIXELS * 3 / 4) strip.setPixelColor(i, strip.Color(255, 109, 40));  // set color to yellow for third quarter of strip
    else strip.setPixelColor(i, strip.Color(234, 4, 126));                             // set color to green for fourth quarter of strip
  }
